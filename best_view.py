"""Stage-3a best-view candidate scoring for RAS instance tracks.

This module does not reimplement the paper's surface-area score.  It bounds and
feeds the unpatched upstream ``compute_surface_area_from_pointmap``, records
every candidate frame it scored, and writes the negotiated
``palatial.best_view.v1`` report.  The naive largest-mask (2D) pick is always
reported next to the 3D pick so a caller can see when the two disagree instead
