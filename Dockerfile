...
# Build on linux/amd64. Prefer baking VGGT weights once or mounting STAGE2_MODELS_DIR.
#
...
ARG VGGT_REVISION=9e4fa662a8893ed348d048e8b57816c12593448b
...
# Pin the RAS and VGGT source trees to the revisions reviewed with this wrapper.
# SAM segmentation is an external fal leg; this image intentionally contains
# no SAM checkout or checkpoint.
...
COPY stage2_service.py handler.py artifact_upload.py artifact_contract.py point_cloud_glb.py object_catalog.py best_view.py /app/
